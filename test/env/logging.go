...
func SetupLoggingForNerdctl(ctx context.Context, name string) (*slog.Logger, context.Context, func() error, error) {
	return SetupLogForwardingToContainerd(ctx, fmt.Sprintf("nerdctl[%s]", name), false)
}

func SetupLoggingForShim(ctx context.Context) (*slog.Logger, context.Context, func() error, error) {
...
func SetupLogForwardingToContainerd(ctx context.Context, shimName string, redirectStderr bool) (*slog.Logger, context.Context, func() error, error) {
	...
	if err != nil {
		return nil, nil, nil, errors.Errorf("dialing raw writer socket: %w", err)
	}
	...
	if err != nil {
		return nil, nil, nil, errors.Errorf("dialing delimited writer socket: %w", err)
	}
	...
	if err != nil {
		return nil, nil, nil, errors.Errorf("configuring otel: %w", err)
	}
	...
	ctx = slogctx.NewCtx(ctx, loggerz)

	return loggerz, ctx, dynamicCleanup.DetachDeferAsCloser(), nil
}
