...
func newDefaultLinuxVM(ctx context.Context, vmid string, mbinFile string, mshareFiles map[string]any, mshareDirs []string, mshareSocks []string, extraPortMappings map[uint16]uint16, extraHostPortsToExposeToGuest map[uint16]uint16) (*DefaultVMConfig, error) {
	start := time.Now()
	...
	netdev, hostIPPort, err := PrepareVirtualNetwork(ctx, extraPortMappings, extraHostPortsToExposeToGuest)
	if err != nil {
