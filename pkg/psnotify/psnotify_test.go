...
func TestWatchFork(t *testing.T) {
	...
	if err := tw.watcher.Watch(pid, psnotify.PROC_EVENT_FORK); err != nil {
		t.Error(err)
	}
...
func TestWatchExit(t *testing.T) {
	...
	if err := tw.watcher.Watch(childPid, psnotify.PROC_EVENT_EXIT); err != nil {
		t.Error(err)
	}
...
func TestWatchFollowFork(t *testing.T) {
	...
	if err := tw.watcher.Watch(pid, psnotify.PROC_EVENT_ALL); err != nil {
		t.Error(err)
	}
